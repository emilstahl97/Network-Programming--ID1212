...
public class OneInAMillion extends AbstractGame implements Servable {
	...
	public static final String GAME_NAME = "One in a Million";
	...
	public void serve(BufferedReader in, PrintWriter out) throws IOException {
		...
		if (checkNewBestScore()) {
			out.println("That's a new best score -- please enter your initials...");
			String initials = in.readLine().trim();
			setBestScore(GAME_NAME, numGuesses, initials);
		}
		out.println("Thanks for playing!");
	}

	private boolean checkNewBestScore() {
		return (getBestScore(GAME_NAME) == null || numGuesses < getBestScore(GAME_NAME).getScore());
	}
	...
	private String buildCongratulationsString() {
		...
		if (numGuesses <= 5)
			...
			start = "Finally!!  ";
		return start += "You guessed it using " + numGuesses + " guesses.";
	}
	...
	private String buildSuggestionString(String cue) {
		String s = cue + "  The answer lies between " + (lowBound) + " and " + (highBound) + ".";
